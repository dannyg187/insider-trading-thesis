#!/usr/bin/env python3
# Re-runs the extraction for just the files named in to_rerun.txt, with a
# sharpened prompt. Recovers the ~130 firms that came back with a null or
# obviously wrong day count despite has_recurring_blackout=True, plus the two
# 80-day outliers. Writes its own csv; 02_merge_recovery.py folds it back in.
#
...
OUTPUT_CSV = "blackout_summary_recovery.csv"
TARGETED_LIST_FILE = Path("to_rerun.txt")

MODEL_NAME = "gpt-5.4-nano"  # stay with this model
MAX_CONTEXT_CHARS = 30000
MAX_RETRIES = 2

# targeted mode reads to_rerun.txt, one filename per line; leave the other
# two off for the real recovery run
TARGETED_MODE = True
TEST_MODE = False
TEST_FILE_LIMIT = 3
RANGE_MODE = False
...
==========================================================
ENHANCED CONVERSION RULES — apply these CAREFULLY before returning null.
==========================================================

(1) DATE-OF-MONTH PATTERNS — "Xth day of the [Y] month of the quarter":
Assume 30-day months. Months of a quarter: first/second/third (= last).
Days-before-quarter-end = (days remaining in quarter after that date).
Formula: if Xth day of FIRST month → 90 - X days before quarter end (RARELY happens)
         if Xth day of SECOND month → 60 - X days before quarter end
         if Xth day of THIRD/LAST month → 30 - X days before quarter end
Examples:
  "10th day of the third month of the quarter" → 30 - 10 = 20
  "15th calendar day of the last month" → 30 - 15 = 15
  "20th day of the last month of the quarter" → 30 - 20 = 10
  "first day of the last month" → 30 - 1 = 29
  "last day of the second month" → 60 - 30 = 30

(2) TRADING DAYS vs CALENDAR DAYS:
The output variable is in CALENDAR days. Convert trading days using factor 1.4
(5 trading days per 7 calendar days). Round to nearest integer.
Examples:
  "10 trading days prior to quarter end" → 14
  "15 trading days prior to quarter end" → 21
  "twenty (20) trading days prior to quarter end" → 28

(3) "APPROXIMATELY X WEEKS/MONTHS" — don't return null; convert:
  "approximately two weeks before quarter end" → 14
  "approximately three weeks before quarter end" → 21
  "approximately one month before quarter end" → 30
  "approximately six weeks before quarter end" → 42
  "X weeks" without "approximately" → X * 7
  "X months" without "approximately" → X * 30

(4) EARNINGS-RELEASE-ANCHORED POLICIES (return null, do NOT guess):
If the blackout/window timing is anchored to the EARNINGS RELEASE date
rather than the QUARTER END date, set blackout_start_days_before_quarter_end = NULL.
Indicators:
  "window opens X days after earnings release"
  "window lasts X days after earnings announcement"
  "blackout begins X weeks before earnings announcement"
  "X business day after public release of ... earnings"
These policies do NOT have a fixed relationship to quarter-end. Set has_recurring_blackout = TRUE
but blackout_start_days_before_quarter_end = null. Mention "anchored to earnings release" in the description.

(5) MONTH-BASED TIMING (original guidance, still applies):
- "closes on last day of second month" → 30 days before quarter end
- "closes at end of second month" → 30 days before quarter end
- "closes first day of last month" → ~29 days before quarter end
- "closes mid last month" → ~15 days before quarter end
- "closes on last day of fiscal quarter" → 0 days

(6) STANDARD CONVERSIONS:
- "two weeks" = 14, "one week" = 7, "one month" = 30
- "prior to" = "before"
...
EXAMPLES OF RECOVERY-RELEVANT CASES:
"15 Trading Days prior to the end of the quarter" → 21 (trading days conversion)
"approximately three weeks before the end of the quarter" → 21 (approximately conversion)
"window closes on the 10th day of the third month" → 20 (date-of-month formula)
"window opens 2 business days after earnings release and lasts 30 days" → null (earnings-anchored)
"window ends approximately seven weeks later" (after earnings release) → null (earnings-anchored)
"trading window is communicated each quarter" → null (truly indeterminate; keep null)

NULL only if: timing is truly indeterminate (announced ad-hoc each quarter) OR
                anchored to earnings release (not quarter end).
...
def targeted_files(all_files):
    # returns None if the list file is missing, so main can bail out
    if not TARGETED_LIST_FILE.exists():
        logging.error(f"TARGETED_MODE is on but {TARGETED_LIST_FILE} does not exist. "
                      f"Create the file with one filename per line and try again.")
        return None

    with TARGETED_LIST_FILE.open("r", encoding="utf-8") as fp:
        wanted = {line.strip() for line in fp if line.strip()}

    files = [f for f in all_files if f.name in wanted]
    missing = wanted - {f.name for f in files}
    if missing:
        logging.warning(f"{len(missing)} filenames listed in {TARGETED_LIST_FILE} were not "
                        f"found in {EX19_DIR}. Sample: {sorted(missing)[:3]}")
    return files
...
def main():
    ...
    if TARGETED_MODE:
        files = targeted_files(all_files)
        if files is None:
            return
    elif RANGE_MODE:
    ...
    for i, f in enumerate(files, 1):
        ...
        row.update({
            ...
            "preclearance_description": result.get("preclearance_description", "").replace("\n", " "),
            "prohibits_hedging": result.get("prohibits_hedging", False),
            "hedging_description": result.get("hedging_description", "").replace("\n", " "),
            "general_description": result.get("general_description", "").replace("\n", " "),
            "group_name": str(primary.get("name", "")),
            "blackout_description": str(pattern.get("description", "")).replace("\n", " "),
            "blackout_start_days_before_quarter_end": "" if days is None else str(days),
