#!/usr/bin/env python3
"""
01b_recover_missing.py – targeted re-extraction for recovery

Goal:
    Re-extract policy info for a specific list of files (one per line in
    to_rerun.txt) using a sharpened prompt. Used to recover the ~130 firms
    where the main script gave a null/wrong blackout-days value despite
    has_recurring_blackout=True, plus the two known 80-day outliers.

Output:
    blackout_summary_recovery.csv (separate from blackout_summary.csv).
    Run 02_merge_recovery.py afterwards to combine the two.
...
# TARGETED MODE: only process filenames listed in TARGETED_LIST_FILE.
# Leave TEST_MODE / RANGE_MODE off for the actual recovery run.
TARGETED_MODE = True
...
def main() -> None:
    ...
    if TARGETED_MODE:
        if not TARGETED_LIST_FILE.exists():
            logging.error(
                f"TARGETED_MODE is on but {TARGETED_LIST_FILE} does not exist. "
        ...
        if missing:
            logging.warning(
                f"{len(missing)} filenames listed in {TARGETED_LIST_FILE} were not "
                f"found in {EX19_DIR}. Sample: {sorted(missing)[:3]}"
            )
        logging.info(
            f"TARGETED MODE: Processing {len(files)} files listed in "
            f"{TARGETED_LIST_FILE} (out of {len(all_files)} total in {EX19_DIR})"
        )
    elif RANGE_MODE:
